...
function Bot({position}) {
  ...
  return (
      ...
      <mesh ref={leftRef} position={position}>

        <mesh scale={[1,.75,1]} ref={domeRef} rotation={[0, 0, 0]} position={[0,2.75,0]} >
          <sphereGeometry args={[.85, 15,15, 1]} />
          <meshStandardMaterial side={THREE.DoubleSide} color="white"  transparent opacity={1} />
        </mesh>
        <mesh scale={[.75,.50,.5]} ref={domeRef} rotation={[0, 0, 0]} position={[0,2.75,0.5]} >
          <sphereGeometry args={[.85, 15,15, 1]} />
